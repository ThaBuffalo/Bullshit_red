...
void atk23_getexp(void)
{
	...
	case GetExp_Calculation:	; // calculate experience points to redistribute
		...
		if (CouldHaveEvolvedViaLevelUp(&gPlayerParty[gBattleStruct->expGetterMonId]))
			evolutionBoost = 12;
if (!FlagGet(FLAG_BADGE01_GET) && pokeLevel >= 16)
cap = 0;
else if (!FlagGet(FLAG_BADGE02_GET) && pokeLevel >= 26)
cap = 0;
else if (!FlagGet(FLAG_BADGE03_GET) && pokeLevel >= 36)
cap = 0;
else if (!FlagGet(FLAG_BADGE04_GET) && pokeLevel >= 46)
cap = 0;
else if (!FlagGet(FLAG_BADGE05_GET) && pokeLevel >= 56)
cap = 0;
else if (!FlagGet(FLAG_BADGE06_GET) && pokeLevel >= 66)
cap = 0;
else if (!FlagGet(FLAG_BADGE07_GET) && pokeLevel >= 76)
cap = 0;
else if (!FlagGet(FLAG_BADGE08_GET) && pokeLevel >= 80)
cap = 0;
else 
		...
		calculatedExp = ExpCalculator(trainerBonus, tradeBonus, baseExp, eggBoost, defLevel, pokeLevel, passPower, affection, evolutionBoost, divisor, cap);
